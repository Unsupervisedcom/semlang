...
These requirements govern SemLang type declarations, typed fields, compiler diagnostics for type metadata, and JSON Schema export.
...
- 01.02.001: The JSON Schema exporter MUST emit a draft 2020-12 document and declare the SemLang vocabulary URI `https://semlang.dev/vocab/semlang/1`.
...
- 01.02.006: `currency` values MUST export as JSON Schema numbers with `x-semlang-primitive: currency`.
...
- 01.02.009: Field `unique` metadata MUST export as `x-semlang-unique: true`.
- 01.02.010: Identity fields MUST export with `x-semlang-identity: true`, and concept schemas MUST list identity field names in `x-semlang-identity`.
