...
Keep the setup minimal: this package loads `pi-mcp-adapter`; the skill only helps manage MCP JSON configuration.
...
8. Mention that SemLang MCP respects `SEMLANG_*` environment settings and that `semlang setup` can inspect resolved SemLang settings.

## Server snippet

Unless a project-local SemLang binary strategy is explicitly requested and is more reliable for the project, add this server entry pinned to the package version:
...
For an existing config file, merge the `semlang` object under the existing `mcpServers` object instead of replacing the file.
