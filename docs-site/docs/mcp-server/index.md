...
- `query.run` emits Malloy and executes it with the Malloy SDK, using the config captured by `set_ontology_source` and a required `query_limit_seconds` deadline.
